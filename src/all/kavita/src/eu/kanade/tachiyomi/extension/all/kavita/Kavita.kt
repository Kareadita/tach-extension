...
class Kavita(
    ...
    private val LOG_TAG = """Kavita_${"[$suffix]_" +
        preferences
            .getString(
                KavitaConstants.customSourceNamePref,
                "[$suffix]",
            )!!
            .replace(' ', '_')
    }"""
    ...
    override fun fetchPopularManga(page: Int) = fetch(popularMangaRequest(page))

    override fun fetchLatestUpdates(page: Int) = fetch(latestUpdatesRequest(page))

    override fun fetchSearchManga(
        page: Int,
        query: String,
        filters: FilterList,
    ): Observable<MangasPage> = fetch(searchMangaRequest(page, query, filters))
    ...
    override fun popularMangaRequest(page: Int): Request = prepareRequest(page, buildFilterBody(MetadataPayload()))
        ...
        if (smartFilterFilter?.state != 0 && smartFilterFilter != null) {
            ...
            client.newCall(request).execute().use {
                if (it.code == 200) {
                    ...
                    val decodedFilter = json.decodeFromString<FilterV2Dto>(it.body.string())
                    decodedFilter.statements.add(FilterStatementDto(FilterComparison.NotContains.type, FilterField.Formats.type, "3"))

                    // Make request with selected filters
                    return prepareRequest(page, json.encodeToJsonElement(decodedFilter).toString())
                } else {
    ...
    override fun fetchMangaDetails(manga: SManga): Observable<SManga> {
        ...
        return client
            .newCall(GET("$apiUrl/series/metadata?seriesId=$serieId", headersBuilder().build()))
            .asObservableSuccess()
            .map { response ->
    ...
    override fun mangaDetailsParse(response: Response): SManga {
        ...
        val serieDto =
            client
                .newCall(GET("$apiUrl/Series/${result.seriesId}", headersBuilder().build()))
                .execute()
                .parseAs<SeriesDto>()
    ...
    private fun buildFilterBody(filter: MetadataPayload): String {
        val filterDto = FilterV2Dto()
        filterDto.sortOptions.sortField = filter.sorting
        filterDto.sortOptions.isAscending = filter.sorting_asc

        // Fields that support contains and not contains statements
        val containsAndNotTriplets =
            listOf(
        ...
        filterDto.addContainsNotTriple(containsAndNotTriplets)
        ...
        filterDto.addPeople(peoplePairs)

        // Customized statements
        filterDto.addStatement(FilterComparison.Contains, FilterField.Formats, filter.formats)
        filterDto.addStatement(FilterComparison.Matches, FilterField.SeriesName, filter.seriesNameQuery)
        // Hardcoded statement to filter out epubs:
        filterDto.addStatement(FilterComparison.NotContains, FilterField.Formats, "3")
        if (filter.readStatus.isNotEmpty()) {
            filter.readStatus.forEach {
                if (it == "notRead") {
                    filterDto.addStatement(FilterComparison.Equal, FilterField.ReadProgress, "0")
                } else if (it == "inProgress") {
                    filterDto.addStatement(FilterComparison.GreaterThan, FilterField.ReadProgress, "0")
                    filterDto.addStatement(FilterComparison.LessThan, FilterField.ReadProgress, "100")
                } else if (it == "read") {
                    filterDto.addStatement(FilterComparison.Equal, FilterField.ReadProgress, "100")
                }
            }
        }
        // todo: check statement
        // filterDto.addStatement(FilterComparison.GreaterThanEqual, FilterField.UserRating, filter.userRating.toString())
        if (filter.releaseYearRangeMin != 0) {
            filterDto.addStatement(FilterComparison.GreaterThan, FilterField.ReleaseYear, filter.releaseYearRangeMin.toString())
        }

        if (filter.releaseYearRangeMax != 0) {
            filterDto.addStatement(FilterComparison.LessThan, FilterField.ReleaseYear, filter.releaseYearRangeMax.toString())
        }
        return json.encodeToJsonElement(filterDto).toString()
    }
    ...
    private fun androidx.preference.PreferenceScreen.editTextPreference(
        ...
        EditTextPreference(context).apply {
            ...
            setOnPreferenceChangeListener { _, newValue ->
                try {
                    // We don't allow hot have multiple sources with same ip or domain
                    val opdsUrlInPref = opdsUrlInPreferences(newValue.toString())
    ...
    private fun getToggledFilters() =
        preferences.getStringSet(KavitaConstants.toggledFiltersPref, KavitaConstants.defaultFilterPrefEntries)!!
        ...
        client.newCall(request).execute().use {
            ...
            if (it.code == 200) {
                ...
                if (it.code == 500) {
                    Log.e(
                        LOG_TAG,
                        "[LOGIN] login failed. There was some error -> Code: ${it.code}.Response message: ${it.message} Response body: $peekbody.",
                    )
                    throw LoginErrorException(helper.intl["login_errors_failed_login"])
                } else {
                    Log.e(
                        LOG_TAG,
                        "[LOGIN] login failed. Authentication was not successful -> Code: ${it.code}.Response message: ${it.message} Response body: $peekbody.",
                    )
                    throw LoginErrorException(helper.intl["login_errors_failed_login"])
                }
            }
            ...
            Single
                .fromCallable {
                    ...
                    try {
                        ...
                        val serverInfoDto =
                            client
                                .newCall(GET(requestUrl, headersBuilder().build()))
                                .execute()
                    ...
                    try {
                        ...
                        client
                            .newCall(GET("$apiUrl/Metadata/genres", headersBuilder().build()))
                            .execute()
                            .use { response ->
                        ...
                        // tagsListMeta
                        client
                            .newCall(GET("$apiUrl/Metadata/tags", headersBuilder().build()))
                            .execute()
                            .use { response ->
                                tagsListMeta =
                                    try {
                        ...
                        // age-ratings
                        client
                            .newCall(GET("$apiUrl/Metadata/age-ratings", headersBuilder().build()))
                            .execute()
                            .use { response ->
                                ageRatingsListMeta =
                                    try {
                        ...
                        // collectionsListMeta
                        client
                            .newCall(GET("$apiUrl/Collection", headersBuilder().build()))
                            .execute()
                            .use { response ->
                                collectionsListMeta =
                                    try {
                        ...
                        // languagesListMeta
                        client
                            .newCall(GET("$apiUrl/Metadata/languages", headersBuilder().build()))
                            .execute()
                            .use { response ->
                                languagesListMeta =
                                    try {
                        ...
                        // libraries
                        client
                            .newCall(GET("$apiUrl/Library/libraries", headersBuilder().build()))
                            .execute()
                            .use { response ->
                                libraryListMeta =
                                    try {
                        ...
                        // peopleListMeta
                        client
                            .newCall(GET("$apiUrl/Metadata/people", headersBuilder().build()))
                            .execute()
                            .use { response ->
                                peopleListMeta =
                                    try {
                        ...
                        client
                            .newCall(GET("$apiUrl/Metadata/publication-status", headersBuilder().build()))
                            .execute()
                            .use { response ->
                                pubStatusListMeta =
                                    try {
                        ...
                        client
                            .newCall(GET("$apiUrl/filter", headersBuilder().build()))
                            .execute()
                            .use { response ->
                                smartFilters =
                                    try {
